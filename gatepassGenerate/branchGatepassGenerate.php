...
$product = $db->getRow("SELECT * FROM `product` where `chassis_no`=?",array($id));
if(!empty($product)){
...
$gatepass_id = str_pad($product['product_id'], 3, "0", STR_PAD_LEFT)."-B";
$created_at = date("d-m-Y h:i A",strtotime($product['updated_at']));
// Find Branch Name
...
<h5 style="text-align:right;">Date - $created_at</h5>
Name of Branch : <u>$name</u><br>
