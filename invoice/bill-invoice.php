...
if(isset($_REQUEST['id']) && !empty($_REQUEST['id'])){
	...
	{
		// find billing data
		$select = $db->getRow("SELECT * FROM `billing` where `billing_id`=?",array($bill_id));
		if(!empty($select))
		{
			//Product detail
			...
			if(!empty($product))
			{
				...
				$customer = $db->getRow("SELECT * FROM `customer_detail` where `customer_detail_id`=?",array($select['customer_detail_id']));
				if(!empty($customer))
				{
					// customer payment
					$customer_payment = $db->getRows("SELECT * FROM `customer_payment` where `customer_detail_id`=?",array($customer['customer_detail_id']));
					//gatepass
					$gatepass = $db->getRow("SELECT * FROM `gatepass` where `gatepass_id`=?",array($select['gatepass_id']));
					if(!empty($gatepass))
					{
						// find salesman name
...
<html>
    ...
    <div class="invoice">
        ...
        <div class="invoice-details">
            Invoice No: <?php echo str_pad($select['billing_id'], 3, "0", STR_PAD_LEFT)."-01";?>
            <br />
        ...
        <div class="clear-fix"></div>
            ...
            <?php
			$total = $grand_total = 0;$vat = 12.5;$add = 2.5;		
			$rto = explode("/",$product_price['rto']);
			$ins = explode("/",$product_price['insurance']);
			$productDetail = $product['model']." - ".$product['color']."<br>\tChassis No. - ".$product['chassis_no']."<br>\tEngine No. - ".$product['eng_no']."<br>\tKey No. - ".$product['key_no'];
			...
			if(strtolower($rto[0])=="no"){
				echo("<tr>");
				echo("<td colspan='3' class='text-right'>CRTM</td>");
				echo("<td class='text-right'>Rs " . $rto[1] . "</td>");
				echo("</tr>");
			}else{
				echo("<tr>");
				echo("<td colspan='3' class='text-right'>R.T.O.</td>");
				echo("<td class='text-right'>Rs " . $rto[1] . "</td>");
				echo("</tr>");
			}
			echo("<tr>");
            echo("<td colspan='3' class='text-right'>Insurance</td>");
            echo("<td class='text-right'>Rs " . $ins[1] . "</td>");
            echo("</tr>");
			echo("<tr>");
            echo("<td colspan='3' class='text-right'>AMC</td>");
            echo("<td class='text-right'>Rs " . $product_price['amc'] . "</td>");
            echo("</tr>");
			/*echo("<tr>");
			echo("<td colspan='3' class='text-right'>Smart No Plate</td>");
            echo("<td class='text-right'>Rs " . $product_price['no_plate_fitting'] . "</td>");
            echo("</tr>");*/
			echo("<tr>");
            echo("<td colspan='3' class='text-right'>RMC Veh.Tax</td>");
            echo("<td class='text-right'>Rs " . $product_price['rmc_tax'] . "</td>");
            echo("</tr>");
			echo("<tr>");
            echo("<td colspan='3' class='text-right'>Ex.Warrenty</td>");
            echo("<td class='text-right'>Rs " . $product_price['ex_warranty'] . "</td>");
            echo("</tr>");
            echo("<tr>");
            echo("<td colspan='3' class='text-right'><b>TOTAL</b></td>");
			
			$vatTotal = $total*$vat/100;
			$addTotal = $total*$add/100;
			
			$grand_total = $vatTotal+$addTotal+$rto[1]+$ins[1]+$product_price['amc']+$product_price['no_plate_fitting']+$product_price['rmc_tax']+$product_price['ex_warranty']+$total;
			
			if($rto[1]<=0){
				//$grand_total = $grand_total+550;
			}
			...
			<div class="note">
				<b>Note</b> : The above vehicle have delivered as per my satisfaction level and checked by me and found ok. &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
				<?php if($product_price['discount']>1){?>
					<?php echo "Rs. ".$product_price['discount']."/DIS";?>
				<?php } ?>
				<br><br><br><br>
